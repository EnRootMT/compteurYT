...
def demo(n, block_orientation, rotate, inreverse):
    ...
    while i == 0 :
        ...
        url = 'http://api.openweathermap.org/data/2.5/weather?q=VILLE&appid=CLE_API_METEO&units=metric'
        responsemeteo = requests.get(url)
        data = json.loads(responsemeteo.text)

        temp = data['main']['temp']
        tempdisplay = str(temp)
        ...
        while j < 4 :
            ...
            with canvas(device) as draw:
                text(draw, (0, 0), tempdisplay, fill="white", font=proportional(CP437_FONT))
            time.sleep(5)
